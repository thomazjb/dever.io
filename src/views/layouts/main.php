        ...
        <div class="flex h-screen overflow-hidden">
            ...
            <aside id="sidebar" class="sidebar-transition w-64 bg-dark-900 text-white flex flex-col shadow-xl">
                ...
                <nav class="flex-1 p-4 space-y-1 overflow-y-auto">
                    ...
                    <!-- Tarefas Pendentes -->
                    <a href="<?= Url::to(['/my-tasks', 'status' => 'pending']) ?>"
                    ...
                    <!-- Tarefas Atrasadas -->
                    <a href="<?= Url::to(['/my-tasks', 'overdue' => '1']) ?>"
                        class="nav-link flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-slate-400 hover:text-white hover:bg-white/5">
                ...
                <!-- Perfil do usuário e logout na sidebar -->
                <div class="p-4 border-t border-white/10 flex items-center justify-between">
                    <!-- Perfil do usuário (clicável) -->
                    <a href="<?= Url::to(['/auth/profile']) ?>" class="flex items-center gap-3 hover:bg-white/5 transition-colors">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-xs text-slate-300 truncate"><?= Html::encode($user->email ?? '') ?></p>
                        </div>
                    </a>
                    
                    <!-- Logout button -->
                    <form method="post" action="<?= Url::to(['/auth/logout']) ?>" class="inline-block">
                        <?= Html::hiddenInput(Yii::$app->request->csrfParam, Yii::$app->request->csrfToken) ?>
                        <button type="submit" class="flex items-center gap-2 text-slate-400 hover:text-red-400 text-sm font-medium px-3 py-2 rounded-lg hover:bg-white/5 transition-colors" title="Sair">
